...
def create_dashboard(weather):
    ...
    try:
        ...
        font_val = ImageFont.truetype(FONT_LIGHT, 24)
        font_small = ImageFont.truetype(FONT_BOLD, 18)
        font_tiny = ImageFont.truetype(FONT_LIGHT, 14)
    except: return img
    ...
    draw.text((380, 160), feels_str, fill=TEXT_COLOR, font=font_small, anchor="mm")
    draw.text((380, 190), weather['summary'], fill=TEXT_COLOR, font=font_header, anchor="mm")
    ...
    # 4. MIDDLE: Graph (Shifted Down for Spacing)
    if 'hourly_temps' in weather:
        # Title: y=220 -> 225
        draw.text((40, 225), "24hr Trend", fill=TEXT_COLOR, font=font_small)
        
        # L/H: y=220 -> 225 (Added spacing above)
        low = min(weather['hourly_temps'])
        high = max(weather['hourly_temps'])
        draw.text((760, 225), f"L:{low}°  H:{high}°", fill=TEXT_COLOR, font=font_small, anchor="rs")

        # Graph Box: y=245 -> 250
        graph_box = (40, 250, 760, 315)
        draw_graph_hatching(draw, weather['hourly_temps'], graph_box)

    # 5. BOTTOM: Forecast
    draw.line([(40, 330), (760, 330)], fill=LINE_COLOR, width=3)
    start_x = 50
    for i, day in enumerate(weather['forecast']):
        x = start_x + (i * 150)
        draw.text((x + 40, 350), day['day'], fill=TEXT_COLOR, font=font_small, anchor="mm")
        day_icon = get_icon_image(day['icon_name'], size=(85, 85))
        img.paste(day_icon, (x, 365), day_icon)
        draw.text((x + 40, 460), f"{day['high']}° / {day['low']}°", fill=TEXT_COLOR, font=font_small, anchor="mm")
